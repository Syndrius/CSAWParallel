function ffs_convergence_inputs(Nfs, Nss, mtarg, ntarg, zrtarg, dir)
    ...
    for i in 1:length(Nfs)
        ...
        mkpath(dir * "/ffs"*string(i))
        inputs_to_file(prob=prob, grids=grids, solver=solver, dir=dir*"/ffs"*string(i) *"/")
    end


end

function ffs_convergence(Nfs, Nss, mtarg, ntarg, zrtarg, dir_base)
    ...
    for i in 1:length(Nfs)
        ...
        for zr in zrtarg, m in mtarg, n in ntarg
            ...
            for (j, ind) in enumerate(mat)
                ...
                if zero_crossings(ϕft[:, m+1, n, 1], Nfs[i]) == zr
        ...
        for k in 1:length(eval_true), j in 1:Nint, l in 1:Nint
            ...
            intphi[k, j, l] = MID.Mapping.hermite_interpolation(int_grid[j], int_grid[l], ϕ[:, :, mn_true[k][2], :], xgp, ygp)
        end
        ...
        for k in 1:length(eval_true)
            ...
            #the 1's here are probably wrong, but we don't really care about this case tbh.
